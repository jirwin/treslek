...
WebhookServer.prototype.start = function() {
  ...
  function onRequest(req, res) {
    var pathname = url.parse(req.url).pathname,
        channel = [self.channelKey, pathname.slice(1)].join(':'),
        body = "";
    ...
    req.on('end', function() {
      ...
      async.parallel([
        self.redis.publish.bind(self.redis, channel, JSON.stringify(obj)),
  ...
  this.server = http.createServer(onRequest).listen(this.config.webhook.port, this.config.webhook.host);
