...
TreslekBot.prototype.executeAdminCommands = function(from, to, text, msg) {
  ...
  plugin[command](this.adminSandbox.bot, to, from, args.join(' '), function(err) {
    if (err) {
      log.error('error executing admin command', {err: err, command: command, from: from, text: text.join(' ')});
      return;
    }
    log.info('Successfully executed admin command', {command: command, from: from, text: text.join(' ')});
  });
};
