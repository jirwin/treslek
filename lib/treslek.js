...
TreslekBot.prototype.start = function() {
  ...
  that.sandbox = new Sandbox(that.conf, that.irc, Object.keys(that.registeredCommands), that.commandUsage);
...
TreslekBot.prototype.executeAdminCommands = function(from, to, text, msg) {
  ...
  if (from === this.conf.nick || this.conf.admins.indexOf(from) === -1 || text.indexOf('!') !== 0) {
    return;
  }
...
TreslekBot.prototype.unloadPlugin = function(pluginFile, callback) {
  ...
  if (plugin.hasOwnProperty('hooks')) {
    plugin.commands.forEach(function(hook) {
...
TreslekBot.prototype.loadPlugins = function(callback) {
  ...
  fs.readdir(this.conf.plugins_dir, function(err, files) {
    ...
    files.forEach(function(file) {
      // Only load js files
      if (file.slice(-3) === '.js') {
        that.loadPlugin(that.conf.plugins_dir + file.slice(0, -3), function(err) {
          if (err) {
            console.log('Error: ' + err);
          }
        });
